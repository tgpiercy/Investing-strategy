...
def run_tactical_chart(ticker):
    try:
        ...
        fig.add_trace(go.Candlestick(
            x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], name="PriceAction"
        ), row=1, col=1)
        ...
        fig.update_layout(
            ...
            margin=dict(l=20, r=20, t=20, b=20), height=650, xaxis_rangeslider_visible=False,
...
with st.spinner("Calibrating Volatility Engines..."):
    ...
    if risk['status'] == 'online':
        r = risk['ratio']
        if r >= 1.0: 
            color, title, sub, b_bg = "#FF4444", "🚨 DEFCON 1: VOLATILITY INVERTED (PANIC)", "Market Makers are pricing an immediate crash. HALT ALL LONG BREAKOUTS.", "rgba(255, 68, 68, 0.1)"
        elif r >= 0.9: 
            color, title, sub, b_bg = "#FFAA00", "⚠️ DEFCON 3: ELEVATED RISK", "VIX Term Structure is flattening. Reduce position sizing.", "rgba(255, 170, 0, 0.1)"
        else: 
            color, title, sub, b_bg = "#39FF14", "🟢 DEFCON 5: NORMAL CONTANGO (CLEAR SKIES)", "Institutional fear is low. Kinetic Radar breakouts have high probability.", "rgba(57, 255, 20, 0.05)"
...
with col1:
    ...
    with st.spinner("Pinging CFTC Servers..."):
        macro_df, date = get_macro_tide()
        if not macro_df.empty:
            for _, row in macro_df.iterrows():
                intensity = row['Intensity (%)']
                is_long = row['Net Position'] > 0
                
                if is_long and intensity >= 10: 
                    card_class, text_color, mandate, m_class = "card-bullish", "#39FF14", "MANDATE: PRIORITIZE LONG BREAKOUTS", "mandate-buy"
                elif not is_long and intensity >= 20: 
                    card_class, text_color, mandate, m_class = "card-bearish", "#FF4444", "MANDATE: AVOID LONGS / SEEK SHORTS", "mandate-sell"
                else: 
                    card_class, text_color, mandate, m_class = "card-neutral", "#8b949e", "MANDATE: NEUTRAL TIDE - TRADE LEVEL TO LEVEL", "mandate-warn"
                
                bias_text = "NET LONG" if is_long else "NET SHORT"
                
                st.markdown(f"""
                <div class='tactical-card {card_class}'>
                    <div>
                        <div style='display: flex; justify-content: space-between; align-items: center;'><div class='asset-title'>{row['Asset']}</div></div>
                        <div class='metric-sub' style='margin-top: 10px;'>
                            <div class='data-row'><span>SMART MONEY BIAS:</span><span style='color: {text_color}; font-weight: bold;'>{bias_text} ({intensity:.1f}% INTENSITY)</span></div>
                        </div>
                    </div>
                    <div class='mandate-box {m_class}'>[ {mandate} ]</div>
                </div>
                """, unsafe_allow_html=True)

with col2:
    st.markdown("<div class='apex-header'>☢️ DEALER MATRIX (GAMMA WALLS)</div>", unsafe_allow_html=True)
    with st.spinner("Scanning Indices & Mega-Caps..."):
        gamma_data = get_gamma_walls()
        for g in gamma_data:
            c_dist, p_dist = g['Dist to Call'], g['Dist to Put']
            
            if c_dist < 0: 
                card_class, m_class, mandate, c_text = "card-squeeze", "mandate-buy", "MANDATE: DEALERS SHORT GAMMA - MAXIMIZE LONGS", f"<span style='color: #39FF14; font-weight: bold;'>⚠️ BREACHED ({abs(c_dist):.2f}% ABOVE)</span>"
            elif p_dist > 0: 
                card_class, m_class, mandate, c_text = "card-bearish", "mandate-sell", "MANDATE: DEALERS LONG GAMMA - HEAVY RESISTANCE", f"{c_dist:+.2f}% away"
            else: 
                card_class, m_class, mandate, c_text = "card-neutral", "mandate-warn", "MANDATE: TRAPPED IN CHOP ZONE - RANGE BOUND", f"{c_dist:+.2f}% away"
                
            p_text = f"<span style='color: #FF4444; font-weight: bold;'>⚠️ BREACHED ({abs(p_dist):.2f}% BELOW)</span>" if p_dist > 0 else f"{p_dist:+.2f}% away"
            
            st.markdown(f"""
            <div class='tactical-card {card_class}'>
                <div>
                    <div style='display: flex; justify-content: space-between; align-items: center;'><div class='asset-title'>{g['Ticker']}</div><div class='price-text'>${g['Price']:.2f}</div></div>
                    <div class='metric-sub' style='margin-top: 10px;'>
                        <div class='data-row'><span>🧱 CALL CEILING: <b style='color:#FFF;'>${g['Call Wall']:.2f}</b></span><span>[{c_text}]</span></div>
                        <div class='data-row'><span>🧲 PUT FLOOR: <b style='color:#FFF;'>${g['Put Wall']:.2f}</b></span><span>[{p_text}]</span></div>
                    </div>
                </div>
                <div class='mandate-box {m_class}'>[ {mandate} ]</div>
            </div>
...
with col3:
    ...
    with st.spinner("Scanning Lieutenants for volume ignition..."):
        ...
        if not radar_df.empty: 
            st.dataframe(radar_df.sort_values(by="Vol Spike (x)", ascending=False).style.format({"Dist to High (%)": "{:+.2f}%", "Vol Spike (x)": "{:.2f}x", "Price": "${:.2f}"}), use_container_width=True, height=200)
        else: 
...
with col5:
    with st.spinner("Calculating Intermarket See-Saw (SPY / DBC)..."):
        rot_data = run_rotation_engine()
        if rot_data['status'] == 'online':
            eq_favored = rot_data['equity_favored']
            ...
            st.line_chart(rot_data['chart'], color=["#58a6ff", "#8b949e"], use_container_width=True)

with col6:
    selected_universe = st.radio("Select Universe:", ["Sectors (S&P 500)", "Subsectors (Industry)", "AI & Tech Infra", "Macro (Assets)", "Global Indices"], horizontal=True, label_visibility="collapsed")
...
with recon_col1:
    ...
    <div style='background: #161b22; border: 1px solid #30363d; border-radius: 6px; padding: 15px; margin-top: 20px;'>
        <b style='color: #58a6ff;'>TACTICAL READ:</b><br><br>
        <span style='color: #39FF14;'>🟢 9/21 EMA Cross:</span> Momentum Ignition<br><br>
        <span style='color: #8b949e;'>⚪ 50 SMA:</span> Baseline Trend / Support<br><br>
        <span style='color: #8A2BE2;'>🟣 Dark Pool Bubble:</span> Institutional Block Trade<br><br>
        <hr style='border-color: #30363d;'>
        <b style='color: #58a6ff;'>VOLUME MATRIX:</b><br><br>
        <span style='color: #39FF14;'>🟢 9 Vol SMA:</span> Kinetic Liquidity Momentum<br><br>
        <span style='color: #58a6ff;'>🔵 20 Vol SMA:</span> Monthly Institutional Float<br><br>
        <span style='color: #8b949e;'>⚪ 50 Vol SMA:</span> Macro Liquidity Trend
    </div>
    """, unsafe_allow_html=True)

with recon_col2:
    with st.spinner(f"Loading orbital chart for {target_chart}..."):
